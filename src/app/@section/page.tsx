export default function About() {
    return (
        <div className="my-6 mx-8 space-y-8">
            ...
            <div>
                <h1 className="text-4xl font-semibold dark:text-zinc-50 text-zinc-800">
                    Hello!
                </h1>

                <div className="rounded-full w-40 h-1.5 dark:bg-primary-400 bg-primary-600 my-3" />
            ...
            <div>
                <h2 className="text-3xl mb-4 dark:text-zinc-50 text-zinc-800">My Expertise</h2>
...
function ExpertiseCard({ title, children, tags }: {
    ...
    return (
        <div className="border dark:border-primary-400/20 dark:bg-primary-300/5 bg-primary-50 border-primary-300 shadow-lg rounded-xl p-4 flex flex-col md:min-h-[200px] min-h-[160px]">
            <h3 className="text-lg font-semibold dark:text-zinc-200 text-primary-950">{title}</h3>
            <p className="text-md dark:text-zinc-400 italic grow text-primary-800">{children}</p>
            <div className="flex flex-row shrink gap-2 mt-3">
                {tags.map((tag) => {
                    return (
                        <a key={tag.name} href={tag.link} className="dark:bg-primary-400/60 bg-primary-500 text-zinc-50 rounded-full text-center px-3">{tag.name}</a>
                    )
                })}
            </div>
        </div>
