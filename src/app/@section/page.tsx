...
export default function About() {
    return (
        <div className="sm:my-6 my-5 sm:mx-8 mx-4 sm:space-y-8 space-y-4">
            ...
            <div>
                {/* <h2 className="mb-4 text-zinc-50 text-zinc-800 font-header font-medium text-2xl">My Expertise</h2> */}
...
function ExpertiseCard({ title, children, tags }: {
    ...
    return (
        <div className="border border-primary-400/20 bg-primary-300/5 shadow-lg rounded-xl p-4 flex flex-col md:min-h-[200px] min-h-[160px]">
            <h3 className="text-zinc-200 font-header text-xl font-semibold">{title}</h3>
            <p className="text-zinc-400 italic grow font-paragraph text-md font-medium">{children}</p>
            <div className="flex flex-row shrink gap-2 mt-3">
                {tags.map((tag) => {
                    return (
                        <a key={tag.name} href={tag.link} className="group bg-primary-400/60 text-zinc-50 rounded-full text-center h-8 w-8 hover:w-auto font-header flex flex-row items-center ">
                            <tag.icon className="ml-1.5 w-5 h-5 fill-zinc-50/90" />
